...
class Test (integrationtest_support.IntegrationTestSupport):
    def test (self):
        self.prepare_integration_test('ignore_should_create_lock_file')
        ...
        with self.fixture() as fixture:
            ...
            fixture.expect('ssh', ['it01.domain', '-O', 'check']) \
                   .then_return(0)
            fixture.expect('ssh', ['it01.domain', '-s', 'backend-service'], 'ignore') \
                   .then_return(0)
            fixture.expect('ssh', ['it01.domain', '-s', 'frontend-service'], 'ignore') \
                   .then_return(0)
            fixture.expect('ssh', ['it01.domain', '-O', 'exit']) \
                   .then_return(0)

        status_return_code = self.execute_command('yadtshell status -v')
        ignore_return_code = self.execute_command('yadtshell ignore service://* -m "ignoring" -v')
        ...
        with self.verify() as verifier:
            ...
            verifier.verify('ssh', ['it01.domain', '-s', 'backend-service'], 'ignore')
            verifier.verify('ssh', ['it01.domain', '-s', 'frontend-service'], 'ignore')
