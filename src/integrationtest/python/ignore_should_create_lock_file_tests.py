    ...
    def test (self):
        ...
        with self.fixture() as when:
            ...
            when.calling('ssh').with_arguments('it01.domain', '-O', 'check') \
                .then_return(0)
            when.calling('ssh').with_arguments('it01.domain', '-s', 'backend-service').and_input('ignore') \
                .then_return(0)
            when.calling('ssh').with_arguments('it01.domain', '-s', 'frontend-service').and_input('ignore') \
                .then_return(0)
